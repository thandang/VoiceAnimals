...
- (void) choose:(id) sender{
    ...
    if(_delegate && [_delegate respondsToSelector:@selector(optionsButton:didSelectItem:)]){
        [_delegate optionsButton:button didSelectItem:(unsigned)tag/subviewTagConstant];
    }
}
...
- (void) calculateExpandedCoordinates{
    ...
    if (CGPointEqualToPoint(_mainBtnCustomOrigin, CGPointZero)) {
        _mainBtnCustomOrigin=CGPointMake((width -_mainButtonImage.size.width)/2, height - _mainButtonImage.size.height - 60.0);
    }
